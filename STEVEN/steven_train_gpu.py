...
# Genration of face images and rppg data
loss_val =[]
loss_bin =[]
mse_1 =[]
mae_1 =[]
...
# Generation of skin map
z = []
skin_label = []
MB = '/home/eng/s/sxs190123/steven/skin_data'
m = os.listdir(MB)
m.sort()
print(len(m))
...
for epoch in range(0,EPOCHS):
    ...
    if (epoch > 0):
        f1 = open('model-stev/loss.txt', 'a')
        
    else:
        if not os.path.exists('model-stev'):
            os.makedirs('model-stev')
        
        f1 = open('model-stev/loss.txt', 'w')
        
    f1.write(str(loss_bin[epoch]) + '\t' + str(loss_val[epoch]) + '\t' + str(mse_1[epoch]) + '\t' + str(mae_1[epoch]) + '\t' + str(mse_2[epoch]) + '\t' + str(mae_2[epoch]) + '\n')
    f1.close()

torch.save(model.state_dict(), 'model-stev/steven_net.pt')
...
plt.xlabel('epoch')
plt.show()
plt.savefig('model-stev/loss_stevnet.png')
...
plt.xlabel('epoch')
plt.show()
plt.savefig('model-stev/binary_loss_stevnet.png')
...
plt.tight_layout()
plt.show()
plt.savefig('model-stev/rppg_error_stevnet.png')
...
plt.tight_layout()
plt.show()
plt.savefig('model-stev/skin_error_stevnet.png')
